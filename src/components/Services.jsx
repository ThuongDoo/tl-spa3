...
function ServiceCard({ item }) {
  ...
  return (
    <div className="group overflow-hidden rounded-3xl border border-sand bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
      <div className="relative aspect-[4/3] overflow-hidden" style={!hasImg ? { background: 'linear-gradient(135deg, var(--c-200), var(--c-50))' } : undefined}>
        {hasImg && <img src={item.img} alt={item.name} className="h-full w-full object-cover" />}
        <div
      ...
      <div className="p-6">
        ...
        <p className="mt-3 text-sm leading-relaxed text-charcoal/70">{item.desc}</p>
        <div className="mt-5 flex items-center justify-between">
          <span className="text-sm font-semibold" style={{ color: 'var(--c-600)' }}>
            {item.price}
          </span>
          <a href="#booking" className="text-sm font-medium text-charcoal/60 transition hover:text-charcoal">
            Đặt ngay →
          </a>
        </div>
      </div>
...
export default function Services({ data }) {
  ...
  const pages = Array.from({ length: totalPages }, (_, i) => items.slice(i * PER_PAGE, i * PER_PAGE + PER_PAGE))
    ...
    <section id="services" className="bg-cream py-24">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-3 font-display text-3xl text-charcoal sm:text-4xl">{data.services.heading}</h2>
          ...
          <div className="mt-8 flex justify-center gap-2">
            {pages.map((_, i) => (
              <button
                ...
                style={{ width: i === page ? '1.5rem' : '0.5rem', background: i === page ? 'var(--c-500)' : 'var(--c-200)' }}
              />
            ))}
          </div>
        )}
